...
class EditInboxAutomation extends EditRecord
{
    protected static string $resource = ClientResource::class;
    protected static ?string $title = 'Inbox Automation';
    protected static ?string $navigationIcon = 'heroicon-m-chat-bubble-left-right';

    public function form(Form $form): Form
    {
        return $form->schema(
            InboxAutomationTab::schema()
        );
    }
            ...
            public static function canAccess(array $parameters = []): bool
    {
        ...
        if ($client) {
            ...
            if ($client->plan) {
                ...
                if (is_array($planHiddenMenus) && in_array('inbox-automation', $planHiddenMenus)) {
            ...
            if (is_array($hiddenMenus) && in_array('inbox-automation', $hiddenMenus)) {
                return false; // Menu is hidden specifically for this client
            }
        }

        if ($user->role === 'staff') {
            return $user->hasStaffPermission('manage_inbox_automation');
        }
