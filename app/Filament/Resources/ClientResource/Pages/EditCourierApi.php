...
use App\Filament\Resources\ClientResource\Schemas\Tabs\CourierApiTab;

class EditCourierApi extends EditRecord
{
    protected static string $resource = ClientResource::class;
    protected static ?string $title = 'Courier API';
    protected static ?string $navigationIcon = 'heroicon-m-archive-box-arrow-down';

    public function form(Form $form): Form
    {
        return $form->schema(
            CourierApiTab::schema()
        );
    }
            ...
            public static function canAccess(array $parameters = []): bool
    {
        ...
        if ($client) {
            ...
            if ($client->plan) {
                ...
                if (is_array($planHiddenMenus) && in_array('courier-api', $planHiddenMenus)) {
            ...
            if (is_array($hiddenMenus) && in_array('courier-api', $hiddenMenus)) {
                return false; // Menu is hidden specifically for this client
            }
        }

        if ($user->role === 'staff') {
            return $user->hasStaffPermission('manage_courier_api');
        }
