...
<head>
            ...
            theme:{
                extend:{
                    colors:{
                        ...
                        primary: 'var(--tw-color-primary)', /* Shoppers Red */
                        gray-900: 'var(--tw-color-primary)', /* Shoppers Dark Blue Head/Foot */
                        gray-50: '#f8f8f8',
                    },
                    fontFamily:{
    ...
    <style>
        :root { --tw-color-primary: {{$client->primary_color ?? "#ef4444"}}; --mob-primary: var(--tw-color-primary); }
        ...
        .cat-item:hover { color: var(--tw-color-primary); padding-left: 20px; background-color: #fcfcfc; }
        ...
        .nav-link:hover { color: var(--tw-color-primary); }
        ...
        .footer-link:hover { color: var(--tw-color-primary); }
    </style>
...
<body class="antialiased flex flex-col min-h-screen font-sans selection:bg-primary/20 selection:text-primary" style="{{ $client->bg_color ? 'background-color: '.$client->bg_color.' !important;' : '' }}">
    ...
    {{-- Top Bar (Dark Blue) --}}
    <div class="bg-gray-900 text-gray-300 text-[11px] py-2 hidden md:block border-b border-gray-700">
        <div class="max-w-[1240px] mx-auto px-4 flex justify-between items-center">
            ...
            <div class="flex items-center gap-5">
                <a href="{{ $client->email ? 'mailto:'.$client->email : '#' }}" class="hover:text-primary transition font-medium">Contact Us</a>
    ...
    <header class="bg-gray-900 py-5 sticky sm:relative top-0 z-50">
            ...
            <div class="flex items-center pl-6 flex-1 gap-1">
                @if(isset($primaryMenu) && $primaryMenu->items->count() > 0)
                    @foreach($primaryMenu->items as $item)
                        <a href="{{ $item->resolved_url }}" target="{{ $item->target }}" class="nav-link {!! request()->is(ltrim($item->resolved_url, '/')) ? '!text-primary border-b-2 border-primary' : '' !!}">{{ mb_strtoupper($item->label) }}</a>
                    @endforeach
                @else
                    <a href="{{$baseUrl}}" class="nav-link !text-primary {!! request()->is('/') ? 'border-b-2 border-primary' : '' !!}">HOME</a>
    ...
    <footer class="mt-auto">
        {{-- Newsletter Bar --}}
        <div class="bg-gray-900 pt-8 pb-6 border-b border-gray-700">
            <div class="max-w-[1240px] mx-auto px-4">
        ...
        <div class="bg-white pt-10 pb-8 border-b border-gray-200">
                    ...
                    <div>
                        ...
                        <div class="space-y-4 mt-4">
                            ...
                            <div class="flex items-start gap-3">
                                <i class="fas fa-map-marker-alt text-primary mt-1 text-sm bg-red-50 p-1.5 rounded-full w-6 h-6 flex items-center justify-center"></i>
                                <span class="text-xs text-gray-500 leading-relaxed">{{ $client->address }}</span>
                            </div>
                            @endif
                            @if($client->phone)
                            <div class="flex items-start gap-3">
                                <i class="fas fa-phone-alt text-primary mt-0.5 text-sm bg-red-50 p-1.5 rounded-full w-6 h-6 flex items-center justify-center"></i>
                                <span class="text-xs text-gray-500 leading-relaxed">Hotline: {{ $client->phone }}</span>
                            </div>
                            @endif
                            @if($client->email)
                            <div class="flex items-start gap-3">
                                <i class="far fa-envelope text-primary mt-0.5 text-sm bg-red-50 p-1.5 rounded-full w-6 h-6 flex items-center justify-center"></i>
            ...
            {{-- Bottom Bar --}}
            <div class="bg-white py-6 border-b-[6px] border-primary">
                    ...
                    <div class="flex flex-col items-end">
                        ...
                        <div class="text-[10px] text-gray-400 flex gap-4">
                            ...
                            @if(!empty($client->footer_links))
                                ...
                                <a href="{{ $fl['url'] ?? '#' }}" class="hover:text-primary transition">{{ $fl['title'] ?? '' }}</a>
                                @endforeach
                            @endif
                        </div>
                    </div>
