...
<head>
    ...
    <meta name="theme-color" content="{{ $client->primary_color ?? '#0ea5e9' }}">
    ...
    <style>
        ...
        @media (max-width: 767px) {
            .mob-nav { background: #111827 !important; border-top-color: rgba(255,255,255,0.08) !important; }
...
<body class="bg-dark text-gray-100 antialiased flex flex-col min-h-screen font-sans" style="{{ $client->bg_color ? 'background-color: '.$client->bg_color.' !important;' : '' }}">
    ...
    <div class="text-white text-center py-1.5 px-4 text-xs font-semibold" style="background-color: var(--tw-color-primary);">
        {!! $client->announcement_text !!}
    </div>
    @endif

    {{-- Header --}}
    <header class="bg-panel border-b border-gray-800 sticky top-0 z-50 shadow-lg">
        <div class="max-w-[100rem] mx-auto px-4 md:px-8">
            <div class="flex items-center gap-4 h-14 md:h-16">

                {{-- Logo --}}
                <a href="{{$baseUrl}}" class="flex items-center gap-2 shrink-0">
                    @if($client->logo)
                        <img src="{{asset('storage/'.$client->logo)}}" class="h-8 w-8 md:h-10 md:w-10 object-contain rounded-md" alt="{{$client->shop_name}}">
                    @endif
                    ...
                    <span class="text-white text-[9px] font-bold px-1.5 py-0.5 rounded-sm uppercase tracking-wider ml-1 hidden sm:inline-block flex-shrink-0" style="background-color: var(--tw-color-primary);">Tech</span>
                </a>
                ...
                <div class="hidden md:flex items-center gap-4">
                    <a href="{{$clean?$baseUrl.'/track':route('shop.track',$client->slug)}}"
                        class="text-xs font-semibold text-gray-400 hover:text-white transition flex items-center gap-2 bg-dark tech-border px-3 py-1.5 rounded-md hover:border-gray-500">
                        <i class="fas fa-crosshairs text-primary"></i> Track Status
                    </a>
                    @if($client->fb_page_id)
                    <a href="https://m.me/{{$client->fb_page_id}}" target="_blank"
                        class="w-8 h-8 rounded-md bg-dark tech-border flex items-center justify-center text-gray-400 hover:text-white hover:border-primary transition">
