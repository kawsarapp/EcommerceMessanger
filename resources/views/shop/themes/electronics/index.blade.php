...
<section class="max-w-[100rem] mx-auto px-4 md:px-8 pt-8 pb-12">
    <div class="w-full h-[50vh] md:h-[60vh] rounded-2xl tech-border overflow-hidden relative group">
        ...
        <div class="absolute inset-y-0 left-0 z-30 flex flex-col justify-center p-8 md:p-16 w-full md:w-2/3">
            <div class="inline-flex items-center gap-2 bg-dark/50 backdrop-blur-md tech-border px-3 py-1.5 rounded-full mb-6 w-fit">
                <span class="w-2 h-2 rounded-full bg-primary animate-pulse"></span>
                <span class="text-[10px] font-bold text-gray-300 uppercase tracking-widest font-mono">System Online</span>
            </div>
            
            <h2 class="text-4xl md:text-6xl text-white font-black tracking-tight mb-6 leading-none">
                {{$client->meta_title ?? 'Upgrade Your Rig.'}}
            </h2>
            <p class="text-gray-400 font-medium mb-8 max-w-md hidden sm:block">Discover the latest cutting-edge hardware and accessories engineered for maximum performance.</p>
            
            <a href="#hardware" class="w-fit bg-primary text-white font-bold px-8 py-3.5 rounded-lg flex items-center gap-3 hover:bg-white hover:text-black transition shadow-[0_0_20px_rgba(14,165,233,0.3)]">
                Browse Components <i class="fas fa-arrow-right"></i>
            </a>
        </div>
    </div>
</section>
...
<div id="hardware" class="max-w-[100rem] mx-auto px-4 md:px-8 py-12">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
        @forelse($products as $p) 
            <a href="{{$baseUrl.'/product/'.$p->slug}}" class="group flex flex-col bg-panel rounded-xl tech-border overflow-hidden tech-glow transition-all duration-300">
                <!-- Image Container -->
                <div class="aspect-square bg-white relative p-4 flex items-center justify-center">
                    @if($p->sale_price)
                        <span class="absolute top-3 left-3 z-10 bg-red-500 text-white text-[10px] font-black px-2 py-1 rounded tracking-wider uppercase shadow-[0_0_10px_rgba(239,68,68,0.5)]">-Save</span>
                    @endif
                ...
                <div class="p-4 md:p-5 flex flex-col flex-1">
                    ...
                    <div class="flex justify-between items-end border-t border-gray-800 pt-3 mt-auto">
                        <div class="flex flex-col">
                            @if($p->sale_price)
                                <del class="text-[10px] text-gray-500 font-mono font-medium">৳{{$p->regular_price}}</del>
                            @endif
                            <span class="font-black text-lg text-white font-mono tracking-tight">৳{{number_format($p->sale_price ?? $p->regular_price)}}</span>
                        </div>
