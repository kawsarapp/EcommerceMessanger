            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $client->widgets['table_order_id'] ?? 'Order ID' }}</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $client->widgets['table_date'] ?? 'Date' }}</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $client->widgets['table_total'] ?? 'Total' }}</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $client->widgets['table_status'] ?? 'Status' }}</th>
                            <th scope="col" class="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $client->widgets['table_action'] ?? 'Action' }}</th>
                        </tr>
                    </thead>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                @php
                                    $orderData = [
                                        ...
                                        'items' => $order->orderItems->map(fn($i) => ['name' => $i->product->title ?? ($client->widgets['table_product_name'] ?? 'Product'), 'qty' => $i->quantity, 'price' => number_format($i->price), 'total' => number_format($i->price * $i->quantity)])
                                    ];
                                @endphp
                    ...
                    <div x-show="showModal" 
                        ...
                        <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center sm:px-6">
                            <h3 class="text-xl leading-6 font-bold text-gray-900" id="modal-title">
                                {{ $client->widgets['modal_order_details'] ?? 'Order Details' }} <span class="text-primary">#<span x-text="selectedOrder?.id"></span></span>
                            </h3>
                        ...
                        <div class="px-6 py-5 sm:p-6 text-left">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                                <div>
                                    <p class="text-sm font-bold text-gray-500 uppercase mb-1">{{ $client->widgets['modal_status'] ?? 'Status' }}</p>
                                ...
                                <div>
                                    <p class="text-sm font-bold text-gray-500 uppercase mb-1">{{ $client->widgets['modal_order_date'] ?? 'Order Date' }}</p>
                                ...
                                <div class="col-span-1 sm:col-span-2 bg-gray-50 p-4 rounded-lg border border-gray-100 shadow-sm">
                                    <p class="text-sm font-bold text-gray-500 uppercase mb-2"><i class="fas fa-map-marker-alt text-primary mr-1"></i> {{ $client->widgets['modal_shipping_address'] ?? 'Shipping Address' }}</p>
                            ...
                            <h4 class="font-bold text-gray-900 border-b pb-2 mb-4">{{ $client->widgets['modal_items_summary'] ?? 'Items Summary' }}</h4>
                            ...
                            <div class="mt-6 pt-4 border-t border-gray-200">
                                <div class="flex justify-between mb-2 text-sm text-gray-600">
                                    <span>{{ $client->widgets['modal_subtotal'] ?? 'Subtotal' }}</span>
                                    <span>৳<span x-text="selectedOrder?.subtotal"></span></span>
                                </div>
                                <div class="flex justify-between mb-2 text-sm text-gray-600">
                                    <span>{{ $client->widgets['modal_delivery_charge'] ?? 'Delivery Charge' }}</span>
                                ...
                                <div class="flex justify-between mb-2 text-sm text-red-500" x-show="selectedOrder?.discount !== '0'">
                                    <span>{{ $client->widgets['modal_discount'] ?? 'Discount' }}</span>
                                ...
                                <div class="flex justify-between mt-3 pt-3 border-t font-bold text-lg text-gray-900">
                                    <span>{{ $client->widgets['modal_total'] ?? 'Total' }}</span>
